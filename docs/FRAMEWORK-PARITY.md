...
| URL state | Compatible query keys when enabled; a link's page kept on arrival and on back/forward; a new search, filter or sort on the first page, a repeated one on its page; a page past the last one moved to the last; isolated in-memory state when disabled | React `url-sync-parity.test.tsx` and `url-page-parity.test.tsx`; Vue `use-table-state.test.ts`, `table-actions-parity.test.ts` and state/catalogue-control suites; `queries` fixtures of `parity.json` in both contract suites; Playwright `e2e/views.spec.ts` |
