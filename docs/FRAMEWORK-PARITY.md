...
URL page indexes remain zero-based. Invalid page sizes fall back to defaults. Both editions open a link on the page it names (`<tableId>-page`) and keep the key in the URL: reading the link's search, filters and sort, on arrival or on back and forward, is not a new query. A search, a column or advanced filter, or a sort changed in the table starts on the first page, in the same URL update, as do saved views, clearing the filters and resetting the view. A write that repeats the current query keeps the page: the shared `tableQueryKey` treats absent and empty alike, a search without its surrounding spaces and objects whatever their key order. A page past the last one, from a link or after rows are removed, moves to the last page once `list` answers (Vue's local rows move at once). React `url-page-parity.test.tsx`, Vue `use-table-state.test.ts` and `table-actions-parity.test.ts`, the `queries` fixtures of `tests/fixtures/parity.json` in both contract suites, and `e2e/views.spec.ts` in both editions cover it. Existing action handlers can keep reading their original names. Aggregation receives the filter join operator too. Both editions accept primitive aggregate results, shown in the column's format like the list fallback, and `{ raw, label }` values, whose label is shown as given.
...
Both editions set only what the URL changed when they read it, on arrival and on back and forward: React parses each key once per query string (nuqs) and skips equivalent writes; Vue compares each value, read the way the table keeps it, with the current one as JSON. Back or forward to the same state, or to another column order, loads neither the rows nor the display modes (File tree, Feed, Calendar, Chart, Map) again. Vue `use-table-state.test.ts` and `yayaw-data-table.test.ts`, React `url-sync-parity.test.tsx` and `e2e/filetree.spec.ts` in both editions cover it.
