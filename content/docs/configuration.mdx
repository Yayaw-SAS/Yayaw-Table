...
- `groupBy` is the column used to split rows into lanes.
...
Kanban uses the same TanStack table instance as table mode, so sorting, filters, selection, row actions, pagination, and saved views keep working. Moving a card calls `actions.update(rowId, { [groupBy]: nextValue })`; your backend remains responsible for validating permissions and state transitions.
