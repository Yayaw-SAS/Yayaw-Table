...
### displayModes
Display modes available to users. Keep the default table-only experience with `['table']`, or add `'kanban'` to show a display switcher above the table.

Type: `Array<'table' | 'kanban'>` | Default: `['table']`

### defaultDisplayMode
Display mode used when neither the URL nor a saved view selects one.

Type: `'table' | 'kanban'` | Default: `'table'`
