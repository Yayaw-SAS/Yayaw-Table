...
Most fields map to one primitive value: `text`, `number`, `date`, `radio`, `select`, `switch`, `textarea`, `url`, `value-type`, or `custom`. The `collection` field is different: it edits an array of objects as a structured UI. It is designed for JSON-like data that would otherwise force every consuming app to build its own mini editor.
