---
title: Formulaires, sections et champs collection
description: Construire des formulaires create/edit avec champs groupés, éditeurs de tableaux natifs, validation et collections imbriquées
---

# Formulaires, sections et champs collection
...
La plupart des champs correspondent à une valeur primitive : `text`, `number`, `date`, `radio`, `select`, `multiSelect`, `switch`, `textarea`, `url`, `value-type` ou `custom`. Le champ `collection` est différent : il édite un tableau d'objets avec une interface structurée. Il est conçu pour les données de type JSON qui forceraient sinon chaque application consommatrice à construire son propre mini éditeur.

## Champs primitifs

Les champs primitifs sont conçus pour être pilotés par configuration, afin que les control planes et les outils MCP puissent générer des formulaires sans React personnalisé pour chaque modèle.

| Type | Rôle |
|---|---|
| `text` | Texte sur une ligne. Utilisez `inputType` pour `email`, `password`, `tel`, `text` ou `url`. |
| `number` | Saisie numérique avec `min`, `max` et `step` optionnels. |
| `date` | Saisie date native qui soumet une chaîne sérialisée `yyyy-MM-dd`. `minDate` et `maxDate` acceptent des objets `Date` ou des chaînes de date sérialisées. |
| `radio` | Énumération à choix unique rendue en radio group. Utilise la même forme d'options que `select`. |
| `select` | Liste déroulante à choix unique pour les valeurs de type énumération. |
| `multiSelect` | Liste de checkboxes à choix multiples pour des tableaux de valeurs parmi un jeu d'options fini. |
| `switch` / `checkbox` | Saisie booléenne, sous forme de switch ou de checkbox. |
| `textarea` | Texte multiligne avec `rows` optionnel. |
| `url` | Saisie d'URL avec support optionnel de prévisualisation de métadonnées. |
| `value-type` | Saisie dynamique pilotée par un champ de type voisin. |
| `custom` | Échappatoire pour des contrôles spécifiques à l'application. |
...
Les champs qui ne sont référencés par aucune section restent rendus après les sections déclarées, dans leur ordre initial. Les noms de champs inconnus sont ignorés, ce qui rend la configuration générée tolérante tout en continuant à rendre chaque champ connu.
