...
- For **image** columns: values are URL strings. Relative app paths such as `/example/products/laptop.svg`, `http(s)` URLs, `blob:` URLs, and base64 `data:image/*` URLs are accepted. Empty or invalid values render a compact fallback.
...
- When `table.enableMultiRowSelection` is enabled, click a selectable row checkbox and then Shift-click another to select or deselect the inclusive range in the current row order. Rows that cannot be selected are skipped.
