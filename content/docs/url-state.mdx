...
The table keeps **sort, filters, pagination, column visibility, display mode, grouping, and pinning** in the URL using [Nuqs](https://nuqs.io). This provides:
...
| `{tableId}-display` | `products-display` | Display mode (`table`, `kanban`, or `gallery`). |
| `{tableId}-kanbanGroupBy` | `products-kanbanGroupBy` | Kanban lane grouping column selected from the toolbar. |
| `{tableId}-gallery` | `products-gallery` | Gallery overrides such as image column, title column, card properties, media ratio, fit, size, and label visibility. |
...
- Display mode, Kanban grouping, and Gallery overrides
- Grouping
- Column pinning
- Page size
...
Gallery views persist display choices only. The saved view stores the selected image/title/property columns and card layout settings, not image data.
