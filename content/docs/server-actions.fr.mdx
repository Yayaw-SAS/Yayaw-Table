...
The `/example` route currently runs in local mode so edits remain interactive without a backend: it uses `app/[locale]/example/lib/products-local-actions.ts` with browser `localStorage` persistence.
