...
- `dateFormat?: string` – fallback legacy au format date-fns pour les colonnes date
...
Quand `numberFormat` n'est pas défini, les nombres sont affichés bruts (sans regroupement des milliers). Un `formatter` custom (s'il est fourni lors de la construction programmatique des colonnes) a priorité sur `numberFormat`.
...
Cette option fonctionne directement depuis `columns.definitions`, sans câblage custom supplémentaire.
...
- Ajoutez `'select'` à `columns.order` et (optionnellement) à `columns.visible` pour l'afficher.
