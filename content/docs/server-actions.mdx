...
Keep your data logic in a server-only module (e.g. `lib/products-server.ts`): list with filter/sort/paginate, create, update, delete, bulk operations. This file must only be imported from Server Actions or other server code.
...
Set these to `true` in your table config so the table does not filter/sort/paginate on the client; it only sends params to your **list** action:
...
The `/example` route in this repo uses the pattern above: Server Actions in `app/example/actions/products.ts`, server logic in `app/example/lib/products-server.ts`, and `getTableActions` returning those actions. You can copy and adapt it for your app.
