...
Return form configuration per form type (e.g. `"products"`, `"products-bulk"`). Used to build create/edit forms and bulk edit. Shape: fields array with `name`, `type`, `label`, `required`, etc. See your codebase or the example app `form-config` for the full type.
