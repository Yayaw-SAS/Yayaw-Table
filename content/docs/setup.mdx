...
### 4. Render `DataTable` in a client component

`getTableConfig` and `getTableActions` are functions, so the component that passes them must be a Client Component.
If you installed via Shadcn registry, import `DataTable` from `@/components/ui/yayaw-table` instead of `yayaw-table`.
...
- Top-level table behavior keys like `enableRowSelection`, `enableSorting`, `enableColumnFilters`, `defaultPageSize`, `pageSizeOptions`, `export`, `bulkExport`, `actionsAsIcons`.
...
| `queryClient` | `QueryClient` | TanStack Query client. If omitted, the table creates one. |
...
- Returning a 0-based `page` in `list`; YaYaw Table sends and expects 1-based page index.
...
- In Next.js App Router, use **NuqsAdapter** and render `DataTable` from a Client Component when passing function props.
