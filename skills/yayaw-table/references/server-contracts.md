...
Answer a `page` past the last one (an old link, rows removed since) with
`meta.totalCount` and `meta.pageCount` and no rows, not an error: the table
then asks for the last page.
