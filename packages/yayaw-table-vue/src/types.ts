...
export type TableListViewConfig = import("./list-view").ListViewSettings;
...
export interface TableAggregateParams {
   ...
   * answer `groups`; `calculations` is then empty. See `chart-model.ts`.
   */
  groupBy?: import("./chart-model").ChartGroupBy[];
  /** Values computed per group, in this order. */
  metrics?: import("./chart-model").ChartMetric[];
...
export interface YayawTableProps<TData extends TableRecord = TableRecord> {
  ...
  details?: import("./record-details").RecordDetailsConfig | false;
