        ...
        return filters.every((filter) => {
            ...
            switch (filter.type) {
                    ...
                    switch (filter.operator) {
                        ...
                        case 'isAnyOf':
                            return (
                                ...
                                (filter.values as any[]).includes(value)
                            )
                        case 'isNot':
                            return value !== filter.values
                        case 'isNoneOf':
                ...
                case 'date': {
                    ...
                    const dateFilter = Array.isArray(filter.values)
                        ? new Date(filter.values[0])
                        : new Date(filter.values as any)
...
function computeFacetedData(
    ...
    _accessors: Record<string, (row: any) => any>
): Record<string, any> {
...
export function useDataTableAdvancedFilters<TData = Record<string, any>>(
    ...
    const { advancedFiltersParam, setAdvancedFiltersFromUI, resetAdvancedFilters } =
