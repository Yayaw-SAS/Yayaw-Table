...
export function useRowDnd<TData>(tableId: string, data: TData[], getRowId: (row: TData) => string) {
    ...
    const [isDragEnabled, setIsDragEnabled] = useAtom(rowDragEnabledAtom(tableId))
    ...
    const orderedData = useMemo(() => {
        ...
        const result = rowOrder.map((id) => dataMap.get(id)).filter(Boolean) as TData[]

        // If we're missing any items that exist in data but not in rowOrder, add them at the end
        const existingIds = new Set(rowOrder)
        const missingItems = data.filter((item) => !existingIds.has(getRowId(item)))
