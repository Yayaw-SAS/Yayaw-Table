...
const TranslationsContext = React.createContext<ReturnType<typeof useTableTranslations>>(
    {} as ReturnType<typeof useTableTranslations>
)

// Create a stable provider component to prevent unnecessary re-renders
const TranslationsProvider = memo(function TranslationsProvider({
    children,
    translations
}: {
    children: React.ReactNode
...
const SortingMenuItems = memo(function SortingMenuItems({
    canSort,
    onSort,
    sortDirection,
    translations
...
function ColumnMenuBase<TData>({
    ...
    const { canFilter, canHide, canSort, isVisible, sortDirection } = columnCapabilities
    ...
    const trigger = useMemo(
        () => (
            <div className="h-full w-full" ref={referenceRef}>
                <MenuTrigger onClick={() => setIsOpen(!isOpen)} translations={translations}>
                    {children}
                </MenuTrigger>
            </div>
        ),
