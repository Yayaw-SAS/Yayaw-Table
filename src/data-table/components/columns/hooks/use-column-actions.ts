 ...
 */
export function useColumnActions<TData extends Record<string, unknown> = Record<string, unknown>>({
    columns: externalColumns,
    tableId
}: UseColumnActionsOptions<TData>) {
    ...
    const processColumnsFn = useCallback(() => {
        ...
        if (!externalColumns?.length || initialized.current || processingColumns.current) {
            return null
        }
        ...
        const processedColumns = externalColumns.map((col, idx) => {
            ...
            // Prioritize existing id or accessorKey for ID generation
            let id = columnDef.id
            const columnWithAccessor = columnDef as ColumnDef<TData> & { accessorKey?: string }
            if (!id && columnWithAccessor.accessorKey) {
                id = String(columnWithAccessor.accessorKey)
            }

            // Fall back to getColumnId if needed
            if (!id) {
                id = getColumnId(columnDef, idx)
            }
        ...
        // Update atoms
        setColumns(processedColumns as DataTableColumnDef<Record<string, unknown>>[])
    ...
    useQuery({
        enabled: !!columnsLength && !initialized.current && !processingColumns.current,
