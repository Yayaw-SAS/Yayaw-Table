...
function FilterChip({
    ...
    return (
        ...
        >
            ...
            {!isEditing && <span className="text-muted-foreground text-xs">{filter.operator}</span>}
...
export function CompactFilterPanel({
    ...
    return (
        <Popover onOpenChange={setIsExpanded} open={isExpanded}>
            <PopoverTrigger asChild>
                <Button className="h-7 gap-1" disabled={disabled} size="sm" variant="outline">
                    <Filter className="h-3 w-3" />
                    {activeFiltersCount > 0 ? `${activeFiltersCount} filters` : 'Filter'}
                </Button>
            </PopoverTrigger>
            <PopoverContent align="start" className="w-auto min-w-96 max-w-screen-sm p-4">
                <div className="space-y-3">
                    <div className="flex items-center justify-between">
