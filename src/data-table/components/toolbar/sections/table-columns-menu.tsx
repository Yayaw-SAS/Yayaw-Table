...
export function TableColumnsMenu({
    ...
    const { columnsConfig, tableConfig } = useTableUIConfig(effectiveTableId)
            ...
            columns
                ...
                .map((col) => {
                    ...
                    const columnConfig = config?.columns?.definitions?.find(
                        (def: { id: string; type?: string; header?: string }) => def.id === col.id
                    )
                    return {
                        ...col,
    ...
    const { hiddenColumns, visibleColumns } = useMemo(() => {
        ...
        const visible = hideableColumns.filter((col) => currentVisibility[col.id] !== false)
        const hidden = hideableColumns.filter((col) => currentVisibility[col.id] === false)
    ...
    const handleDragEnd = useCallback(
        (event: DragEndEvent) => {
            ...
            if (over && active.id !== over.id) {
                // Determine which section we're in (visible or hidden)
                const isVisibleSection = visibleColumns.some(
                    (col) => col.id === active.id.toString()
                )
                const sourceColumns = isVisibleSection ? visibleColumns : hiddenColumns

                // Find the indices
                const oldIndex = sourceColumns.findIndex((col) => col.id === active.id.toString())
                ...
                if (oldIndex !== -1 && newIndex !== -1) {
                    ...
                    // Create a new order array based on the section that was changed
                    let newOrder: string[]
                    if (isVisibleSection) {
                        const visibleIds = newSourceColumns.map((col) => col.id)
                        const hiddenIds = hiddenColumns.map((col) => col.id)
                        newOrder = [...visibleIds, ...hiddenIds]
                    } else {
                        const visibleIds = visibleColumns.map((col) => col.id)
                        const hiddenIds = newSourceColumns.map((col) => col.id)
                        newOrder = [...visibleIds, ...hiddenIds]
                    }
        ...
        [visibleColumns, hiddenColumns, dndKitHandleDragEnd, handleColumnOrderChange]
    )
    ...
    }, [hideableColumns, setVisibilityFromUI, setColumnVisibility, visibilityParam, table])
    ...
    const hasHiddenColumns = useMemo(() => hiddenColumns.length > 0, [hiddenColumns])
    const hasVisibleColumns = useMemo(() => visibleColumns.length > 0, [visibleColumns])
