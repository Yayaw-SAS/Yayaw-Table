...
>({
  columns: _columns = EMPTY_ARRAY as unknown as (
    | ColumnDef<TData>
    | DataTableColumnDef<TData>
  )[],
  data: _initialData = EMPTY_ARRAY as unknown as TData[],
  enableColumnFilters: _enableColumnFilters = true,
  enableMultiRowSelection: _enableMultiRowSelection = true,
  enablePagination: _enablePagination = true,
  enableRowSelection: _enableRowSelection = true,
  enableSorting: _enableSorting = true,
  getRowId: _externalGetRowId,
  manualFiltering: _manualFiltering = false,
  manualPagination: _manualPagination = false,
  manualSorting: _manualSorting = false,
  ...
  const batchUpdate = useCallback((updateFn: () => void) => {
    ...
    if (isBatchingUpdatesRef.current && debounceTimerRef.current !== null) {
  ...
  const {
    ...
    enabled: true,
    tableId,
    tableType: _tableType || tableId,
  });

  // Extract table state for ease of access
  const { columnFilters, columnOrder, columnVisibility, pagination, sorting } =
    state;
