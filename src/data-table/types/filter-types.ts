...
export type FilterOperators = {
  ...
  number: 'equals' | 'greaterThan' | 'lessThan' | 'greaterThanOrEqual' | 'lessThanOrEqual' | 'between' | 'notEquals' | 'isEmpty' | 'isNotEmpty'
  date: 'equals' | 'before' | 'after' | 'between' | 'notEquals' | 'isEmpty' | 'isNotEmpty'
  option: 'is' | 'isNot' | 'isAnyOf' | 'isNoneOf' | 'isEmpty' | 'isNotEmpty'
  multiOption: 'contains' | 'containsAll' | 'containsNone' | 'isEmpty' | 'isNotEmpty'
}
...
export type FilterValues<TType extends ColumnDataType = ColumnDataType> = 
  ...
  TType extends 'number' ? number | [number, number] :
  TType extends 'date' ? Date | [Date, Date] :
  TType extends 'option' ? string | string[] :
  TType extends 'multiOption' ? string[] :
  any

/**
 * Advanced filter model for a single column
 */
export interface AdvancedFilterModel<TType extends ColumnDataType = ColumnDataType> {
 ...
 */
export interface ColumnFilterConfig<TType extends ColumnDataType = ColumnDataType> {
  /** Data type of the column */
  type: TType
  ...
  filterFn?: (value: any, filterValue: FilterValues<TType>, operator: FilterOperators[TType]) => boolean
...
export interface FilterActions {
  /** Add a new filter */
  addFilter: (filter: Omit<AdvancedFilterModel, 'id' | 'createdAt' | 'updatedAt'>) => void
  /** Update an existing filter */
  updateFilter: (filterId: string, updates: Partial<AdvancedFilterModel>) => void
  /** Remove a filter */
 ...
 */
export const FILTER_OPERATORS_LABELS: Record<ColumnDataType, Record<string, string>> = {
  text: {
    contains: 'Contains',
    equals: 'Equals',
...
export const DEFAULT_OPERATORS = {
  text: ['contains', 'equals', 'startsWith', 'endsWith', 'notContains', 'isEmpty', 'isNotEmpty'] as const,
  number: ['equals', 'greaterThan', 'lessThan', 'greaterThanOrEqual', 'lessThanOrEqual', 'between', 'notEquals', 'isEmpty', 'isNotEmpty'] as const,
  date: ['equals', 'before', 'after', 'between', 'notEquals', 'isEmpty', 'isNotEmpty'] as const,
  option: ['is', 'isNot', 'isAnyOf', 'isNoneOf', 'isEmpty', 'isNotEmpty'] as const,
  multiOption: ['contains', 'containsAll', 'containsNone', 'isEmpty', 'isNotEmpty'] as const
} as const 