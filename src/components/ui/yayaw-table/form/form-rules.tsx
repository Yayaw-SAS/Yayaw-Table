...
function IssueText({ issues, label }: { issues: RuleIssue[]; label: Label }) {
  ...
  return first ? (
    <p className="col-span-full text-destructive text-xs" data-rule-issue={first.code}>
...
export function FormRuleEditor({
  ...
  const ruleIssues = issues.filter(
    (issue) =>
      !issue.path ||
      (issue.code === "emptyGroup" && samePath(issue.path, []))
  );
  ...
  return (
    ...
    >
      ...
      <GroupItems group={rule.when} onChange={setWhen} path={[]} rule={context} />
