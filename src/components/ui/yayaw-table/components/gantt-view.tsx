...
import { type PlanningFormatters, planningFormatters } from "../planning/format";
      ...
      {days.map((cell) => (
        <div
          ...
          className={cn(
            ...
            cell.isToday ? "font-semibold text-destructive" : "text-muted-foreground"
          )}
          key={cell.date}
...
function GanttBar({
  ...
  const keyAdjust = (operation: DragOperation) => (event: React.KeyboardEvent) => {
    ...
    onKeyAdjust(
      operation,
      (event.key === "ArrowLeft" ? -1 : 1) * (event.shiftKey ? KEYBOARD_WEEK : 1)
    );
  };
  return (
    <div
...
export function DataTableGanttView<TData extends Record<string, unknown>>({
  ...
  const links = view.showDependencies === false ? [] : timelineLinks(rows, snapshot, geometry);

  return (
    <section aria-label={labels.planning} className={cn("space-y-1", className)}>
      <GanttControls
        busy={busy}
        labels={labels}
        ...
        >
            ...
            return (
              ...
              >
                ...
                <GanttTrack
                  ...
                  geometry={geometry}
                  labels={labels}
                  onDrag={(event, operation) => startDrag(event, key, operation)}
                  onKeyAdjust={(operation, days) =>
                    requestDates(timelineDateMutation(row.task, operation, days))
                  }
                  onOpen={() => session.open(row.task.ref)}
                  row={row}
                  session={session}
          ...
          {links.length ? (
            ...
            >
              ...
              {links.map((link) => (
                <path d={link.d} fill="none" key={link.id} stroke="currentColor" />
              ))}
            </svg>
          ) : null}
        ...
        <DragSurface
          ...
          onRelease={() => {
            ...
            if (target && drag.moved && drag.deltaDays) {
              requestDates(
                timelineDateMutation(target.task, drag.operation, drag.deltaDays)
              );
