...
const validateScalar = (
  ...
  const typeError = dataTypeValueError(["dynamic-value", "dynamicValue", "value-type"].includes(field.type) ? dynamicFieldType(field, context) : field.type, value);
  if (typeError) {
...
const validateCollection = async (
  ...
  for (const [index, item] of value.entries()) {
    ...
    const child = await validateFormFields(
      field.itemFields ?? [],
      item as FieldValues,
      { ...context, values: item as FieldValues }
    );
