  ...
  return (
    ...
    >
      <div className="grid grid-cols-2 gap-3">
        {view.sides.map((side) => (
          <div className="grid min-w-0 gap-1" data-sync-side={side.side} key={side.side}>
      ...
      {view.duplicates ? (
        <p className="text-amber-700 text-xs dark:text-amber-400" data-sync-duplicates>
          {view.duplicates}
        </p>
      ) : null}
      {view.conflictsTitle ? (
...
function PendingConflictsList({
  ...
  return (
    <div aria-busy={busy} className="grid gap-3" data-connector-conflicts>
      ...
      {view.lines.length === 0 ? (
        <p className="text-muted-foreground text-sm" data-connector-conflicts-empty>
          {view.empty}
        </p>
      ) : (
            ...
            >
              ...
              {(["table", "target"] as const).map((side) => (
                <span className="flex min-w-0 justify-between gap-2" key={side}>
                  ...
                  <span className="truncate font-medium">{line[side].value}</span>
                </span>
              ))}
      ...
      {state.error ? (
        <p className="text-destructive text-sm" data-connector-error role="alert">
          {state.error}
        </p>
      ) : null}
      {view.lines.length > 1 ? (
