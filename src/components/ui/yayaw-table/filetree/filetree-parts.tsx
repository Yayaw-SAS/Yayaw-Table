...
export function FileTreeMoveDialog({
  ...
  const [expanded, setExpanded] = useState<Set<string>>(() => {
    ...
    return new Set(parent ? [...controller.crumbs(parent).flatMap((crumb) => (crumb.id ? [crumb.id] : []))] : []);
  });
  ...
  return (
    <FileTreeDialog labelledBy={titleId} onClose={onClose}>
      ...
      <input
        ...
        value={query}
      />
      <ul aria-label={label("folders", { count: rows.length })} className="yayaw-ft-folders">
        {rows.map((row) => {
