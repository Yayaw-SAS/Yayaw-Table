...
export function useAutoPageSize({
  ...
  const automatic = enabled && (mode.resetKey === resetKey ? mode.automatic : defaultAutomatic);
  ...
  useEffect(() => {
    setMode((previous) =>
      !enabled || previous.resetKey !== resetKey
        ? { automatic: enabled && defaultAutomatic, resetKey, expectedSize: undefined, fitKey: "" }
        : previous
    );
  ...
  useEffect(() => {
    ...
    previousSize.current = pageSize;
  }, [automatic, pageSize, mode.expectedSize, mode.resetKey, resetKey, setMode]);
