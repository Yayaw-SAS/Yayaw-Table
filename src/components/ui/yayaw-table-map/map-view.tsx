...
import { useIsMobile } from "@/src/components/ui/yayaw-table/hooks/use-mobile";
import type { DisplayModeRenderContext } from "@/src/components/ui/yayaw-table/types/display-mode-renderer";
import type { LocationBounds } from "@/src/components/ui/yayaw-table/utils/location-model";
import {
  attachMapItems,
  BLANK_MAP_STYLE,
  boundsFromMap,
...
type Label = (key: MapLabelKey, params?: Record<string, number | string>) => string;
      ...
      <MarkerContent>
        <button
          ...
          className={cn(
            ...
            (highlighted || selected) && "scale-150 ring-2 ring-[var(--yayaw-map-marker)]/40"
          )}
...
function RecordPopup({
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() =>
      document
        .querySelector<HTMLElement>(`[data-map-open="${CSS.escape(marker.id)}"]`)
        ?.focus()
    );
    ...
    >
      <div className="flex items-center gap-2 border-b px-3 py-2">
        <h3 className="min-w-0 flex-1 truncate font-medium text-sm">
          {label("list")}
          <span className="ml-2 font-normal text-muted-foreground" data-map-in-view>
        ...
        >
          {open ? (
            <ChevronDown aria-hidden="true" className="size-4 md:rotate-[-90deg]" />
          ) : (
          ...
          {shown.map((marker) => (
            <li key={marker.id}>
              <button
                ...
                onBlur={() => onHighlight(undefined)}
                onClick={(event) => onFocusMarker(marker.id, event.detail === 0)}
                onFocus={() => onHighlight(marker.id)}
                onMouseEnter={() => onHighlight(marker.id)}
                onMouseLeave={() => onHighlight(undefined)}
                type="button"
...
export function MapView({ context }: { context: DisplayModeRenderContext }) {
  ...
  const [selection, setSelection] = useState<{ id: string; keyboard: boolean }>();
  ...
  const queryKey = JSON.stringify([context.listParams, settings.locationColumn]);
  useEffect(() => {
  ...
  }, [fitToMarkers, loaded.bounds, loaded.loading, queryKey, ready, settings.initialView]);
  ...
  return (
    ...
    >
      <MapStatus
        ...
        label={label}
        noStyle={!style}
        notices={mapNotices(loaded.result, maxRows, context.locale, (key, fallback) =>
      ...
      <div className="relative flex h-[32rem] overflow-hidden rounded-md border max-md:h-[70vh]">
        <div className="relative min-w-0 flex-1" data-map-canvas="">
            ...
            <MapCanvas
              ...
              styles={
                style
                  ...
                  : { light: BLANK_MAP_STYLE as never, dark: BLANK_MAP_STYLE as never }
              }
          ...
          {webgl ? (
            <MapButtons label={label} mapRef={mapRef} onFit={() => fitToMarkers(true)} />
          ) : null}
