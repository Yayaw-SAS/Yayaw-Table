...
- `https://table.yayaw.app/r/yayaw-table-dashboard.json` (React) et `https://table.yayaw.app/r/yayaw-table-vue-dashboard.json` (Vue) : tableaux de bord (`YayawDashboard`) avec gridstack.js, chargé à la demande. Les widgets affichent des vues enregistrées de n'importe quelle table, des nombres et des notes ; l'hôte stocke les tableaux de bord via `actions.dashboards` et fournit ses tables dans `tables`. Rien ne défile dans un widget par défaut : les enregistrements qui ne tiennent pas laissent place à « +N de plus · Tout voir » (`settings.overflow: "scroll"` pour faire défiler), les graphiques remplissent leur widget et les nombres peuvent se comparer à la période précédente avec une courbe de tendance. `locale` et `tableTranslations` (les libellés de table de la page) sont transmis à chaque widget ; sur téléphone, les nombres prennent la hauteur de leur contenu et les graphiques une hauteur proportionnelle à la largeur. Les documents sont au format JSON version 2 (sections de grilles et zones pleine largeur, vues intégrées, textes par langue) : l'hôte les valide sur son serveur et les décrit aux outils d'IA avec le module pur `dashboard-schema.ts` (`validateDashboard`, `checkDashboardReferences`, `dashboardJsonSchema`), voir [docs/DASHBOARD-SCREENS.md](../docs/DASHBOARD-SCREENS.md).
- `https://table.yayaw.app/r/yayaw-table-map.json` (React, [mapcn](https://www.mapcn.dev/) sur MapLibre GL : l'item dépend de `https://mapcn.dev/r/map.json` et de `maplibre-gl`) et `https://table.yayaw.app/r/yayaw-table-vue-map.json` (Vue, MapLibre GL directement, même rendu) : vue Carte (marqueurs d'une colonne `location`, regroupements, bulles, liste des enregistrements visibles, « Rechercher dans cette zone »). Passez `mapRenderer` à `displayModeRenderers` et ajoutez `"map"` à `displayModes`. Le fond de carte vient de l'hôte (`table.map.style` ou `table.map.styles`) : aucune tuile ni clé d'API n'est livrée. Le worker de MapLibre se charge depuis unpkg ; pour l'héberger vous-même (CSP stricte, hors ligne), copiez `maplibre-gl-worker.mjs` et `maplibre-gl-shared.mjs` depuis `node_modules/maplibre-gl/dist/` dans un même dossier et indiquez le worker dans `table.map.workerUrl` (depuis MapLibre 6, le worker importe le fichier partagé situé à côté de lui).
