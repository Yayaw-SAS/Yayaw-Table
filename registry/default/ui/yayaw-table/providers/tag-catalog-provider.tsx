...
function withCatalogs<T>(
  ...
  const next = withTagCatalogOptions(definitions as { id: string }[], catalogs);
  if (next === definitions) {
    return config;
  }
...
export function useTagCatalogRuntime<TConfig>({
  ...
  const catalogs = useMemo(
    () =>
      Object.fromEntries(
        columns.map((column, index) => [column.columnId, combined.data[index]])
  ...
  const api = useMemo((): TagCatalogApi | undefined => {
    ...
    const keyOf = (columnId: string) => tagCatalogQueryKey(scopeOf(columnId));
    ...
    return {
      ...
      coloredTags: (columnId) => {
        const definition = (
          definitions as { id?: string; coloredTags?: boolean }[] | undefined
