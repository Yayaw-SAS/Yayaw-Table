...
export function TableFiltersMenu({
  ...
  const customFilters = config.columns.definitions
    .filter(
      (column) => column.enableFiltering !== false && column.filterRenderer
    )
    .map((column) => (
      <div key={column.id}>
        {column.filterRenderer?.({
          ...
          onChange: (value) => {
            const rest = columnFilters.filter(
